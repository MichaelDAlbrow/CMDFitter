...
fig, ax = plt.subplots(3,5,figsize=(9,5),sharex='col',sharey='row')
...
for ext, model, f in zip(exts,models,freeze):
    ...
    ax[0,col].text(0.96,3.6,f"Model {model}",horizontalalignment='right',verticalalignment='top')

    ax[0,col].set_ylim((0,4))

    ax[0,col].set_xlim((0,1))
    ax[1,col].set_xlim((0,1))
    ax[2,col].set_xlim((0,1))
    ax[1,col].set_ylim((-30,30))
    ax[2,col].set_ylim((0,1.0))
...
for i in range(1000):
    ...
    args = p[3:9].tolist()
    args.append(fitter.M_ref)
    ax[0,4].plot(q,fitter.q_distribution(q,args),'b-',alpha=0.01)
    ...
    for j in range(101):
        ...
        y[j,i] = p[9] * ((1.0  - fitter.int_sl_0(q[j])) + \
                           p[3]*(0.0 - fitter.int_sl_1(q[j])) + \
                           p[4]*(0.0 - fitter.int_sl_2(q[j])) + \
                           p[5]*(0.0 - fitter.int_sl_3(q[j])) )
...
ax[2,4].fill_between(q,y1=yq1,y2=yq5,color='b',alpha=0.1)
ax[2,4].fill_between(q,y1=yq2,y2=yq4,color='b',alpha=0.4)
ax[2,4].plot(q,yq3,'r-')

ax[0,4].text(0.96,3.6,f"Prior",horizontalalignment='right',verticalalignment='top')
...
ax[2,4].set_xlim((0,1))
ax[0,4].set_ylim((0,4))
ax[1,4].set_ylim((-30,30))
ax[2,4].set_ylim((0,1.0))
...
plt.savefig('fb_q_dist_NS5_prior.png')
